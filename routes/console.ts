...
app.get("/session", BrowserMiddleware.restrictedBrowser(), AuthMiddleware.isAuthenticated(), function (req:Request, res:Response) {
  res.json({
    success: true,
    data: req['user']
  });
...
app.get("/logs", BrowserMiddleware.restrictedBrowser(), function (req:Request, res:Response) {
